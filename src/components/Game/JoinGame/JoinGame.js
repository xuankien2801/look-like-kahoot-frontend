    ...
    <div className={styles.page}>
      {!isPlayerAdded ? (
        <div className={styles.section}>
          <h2>{isLanguageEnglish ? "Join game" : "Tham gia game"}</h2>
          <input
            type="text"
            ref={pinRef}
            placeholder={
        ...
        <div className={styles.section}>
          <h2>
            {isLanguageEnglish ? "You joined the game" : "Bạn đã tham gia vào game"}
          </h2>
          <h4>
            {isLanguageEnglish ? "Waiting on a host to start the game" : "Chờ người tổ chức bắt đầu game"}
          </h4>
          <CircularProgress />
        </div>
      )}
